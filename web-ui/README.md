...
>
> **fix(test): `npm test` no longer overwrites your real `config/profile.yml` / `data/scan-history.tsv`.** A test (`critical-fixes.test.mjs`) imported `prompts.mjs` (→ `paths.mjs`) at the top of the file, so `PROJECT_ROOT` resolved to the **real** parent before the test set `CAREER_OPS_ROOT` to a temp dir — and `PUT /api/profile` leaked an "Acceptance Test" fixture into your profile on every run. Now the carrier is loaded via dynamic `import()` after the env is set, and `tests/test-root-isolation.test.mjs` guards the whole suite against the pattern. No production-code change.
>
> _Full suite **1086/1086** green · i18n + docs synced across all 9 locales._
